...
async function carregarCombos() {
    ...
    document.getElementById('pessoaSelect').innerHTML = '<option value="">Selecione</option>' +
        pessoas.map(pessoa => `<option value="${Number(pessoa.id)}">${AtendeLabApi.escape(pessoa.nome)}</option>`).join('');

    document.getElementById('tipoSelect').innerHTML = '<option value="">Selecione</option>' +
        tipos.map(tipo => `<option value="${Number(tipo.id)}">${AtendeLabApi.escape(tipo.nome)}</option>`).join('');
}
    ...
    try {
        ...
        tbody.innerHTML = atendimentos.map(atendimento => {
            ...
            return `<tr>
                ...
                <td><span class="badge ${classeStatus}">${AtendeLabApi.escape(atendimento.status)}</span></td>
                <td class="text-end">
                    <button class="btn btn-sm btn-outline-primary" onclick="abrirStatus(${Number(atendimento.id)}, '${AtendeLabApi.escapeAttr(atendimento.status)}')">Status</button>
                </td>
            </tr>`;
        }).join('');
    } catch (error) {
